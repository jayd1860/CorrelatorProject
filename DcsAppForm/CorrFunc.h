...
public:
    ...
    // -----------------------------------------------------------------------------
    void setMusp(double val)
    {
        musp = val;

        // re-calculate intermediate common variables
        calcCommonVars();
    }


    // -----------------------------------------------------------------------------
    void setMua(double val)
    {
        mua = val;
...
public:
	...
	CorrFunc()
	{
        ...
        gmean = new double[nbins];
        memset(gmean, 0, sizeof(double)*nbins);
	...
	// -----------------------------------------------------------------------------
	void calcAutocorrCurve(unsigned long long* y, int size)
	{
		if(size<1)
			return;
	...
	// -----------------------------------------------------------------------------
	void calcBloodFlowIndex(double* BFi, double* beta, double* err)
	{
		//BFi_obj->bruteForce(gmean, tmean, nbins);
		BFi_obj->levmar(gmean, tmean, nbins);
		*BFi = BFi_obj->getBFi();
	...
	void mylogbin2()
	{
		...
		for(int ii=0; ii < nbins; ii++)
		{
			...
			gmean[ii] = gbinsum[ii] / t_binmean[ii];
		}
	}

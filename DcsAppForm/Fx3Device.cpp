...
Fx3Device::Fx3Device()
{
	...
	m_clockSpeed = 1.5e8;
	m_dRate = 50000.0;
    m_FriendlyName = new char[100]();

	m_names = new wchar_t*[10];
	for(ii=0; ii<10; ii++)
		m_names[ii] = new wchar_t[256]();
    ...
    if(m_USBDevice != NULL)
    {
        ...
        for(ii=0; ii<m_deviceCount; ii++)
        {
            m_USBDevice->Open(ii);
            ConvertFriendlyName();
            convertCharToWchar(s, m_FriendlyName, strlen(m_FriendlyName));
            wsprintf(m_names[ii], L"(%d)  %s", ii+1, s);
        }

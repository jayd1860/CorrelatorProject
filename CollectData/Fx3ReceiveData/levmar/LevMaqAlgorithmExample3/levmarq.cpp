...
void levmarq_init(LMstat *lmstat)
{
	lmstat->verbose     = 1;
	lmstat->max_it      = 10000;
	lmstat->init_lambda = 5;
	lmstat->up_factor   = 10;
	lmstat->down_factor = 10;
	lmstat->target_derr = 0.0001;
}
			...
			LMstat*  lmstat)
{
	...
	g = new double[npar];
	d = new double[npar];
	delta = new double[npar];
	newpar = new double[npar];
	...
	for(it = 0; it < nit; it++) {
		...
		while(ill && (it < nit)) {
            ...
            if(verbose) 
			{
                // printf("it = %0.3d,  lambda = %0.3g,  err = %0.3g, derr = %0.3g\n", it, lambda, err, derr);
				printf("%d: par[0] = %0.3g, par[1] = %0.3g, err = %0.3g, derr = %0.3g\n", it, par[0], par[1], err, derr);
			}
			if(ill) {

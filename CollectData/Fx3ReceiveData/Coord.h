...
struct Markers
{
	...
	// ---------------------------------------
	int MakeNewX(MyVector* p, int size, double dRate)
	{
		int incr_x = size / num_x;
		for(int ii=0; ii < num_x; ii++)
			text_x[ii] = p[ii*incr_x].x / dRate;
...
struct MyCoord
{
	...
	// -----------------------------------------------------------
	void ResetYMaxMin(MyVector m1, MyVector m2)
	{
		min = m1;
		max = m2;
		range.y = max.y - min.y;
	...
	// -----------------------------------------------------------
	void ResetXMaxMin(MyVector m1, MyVector m2)
	{
		min = m1;
		max = m2;
		range.x = max.x - min.x;
